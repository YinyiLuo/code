...
class BagelConfig(PretrainedConfig):
    def __init__(
        ...
        cross_modal_recon_vit_mask_prob=0.0,
        cross_modal_recon_vae_mask_prob=0.0,
        **kwargs
    ):
        ...
        self.cross_modal_recon_vit_mask_prob = cross_modal_recon_vit_mask_prob
        self.cross_modal_recon_vae_mask_prob = cross_modal_recon_vae_mask_prob
...
class Bagel(PreTrainedModel):
    ...
    def __init__(self, language_model, vit_model, config: BagelConfig):
        ...
        if config.cross_modal_recon:
            if config.visual_und:
                self.vit_recon_head = nn.Linear(
                    self.hidden_size,
                    self.vit_patch_size * self.vit_patch_size * 3,
                )
            if config.visual_gen:
                self.vae_recon_head = nn.Linear(
                    self.hidden_size,
    ...
    def forward(
        ...
        if self.config.cross_modal_recon and self.training:
            if self.config.visual_und and packed_vit_tokens is not None and packed_vit_token_indexes is not None:
                num_vit_tokens = packed_vit_token_indexes.shape[0]
                if num_vit_tokens > 0 and self.config.cross_modal_recon_vit_mask_prob > 0:
                    vit_mask = torch.rand(num_vit_tokens, device=device) < self.config.cross_modal_recon_vit_mask_prob
                    if not vit_mask.any():
                        vit_mask[torch.randint(0, num_vit_tokens, (1,), device=device)] = True
            if self.config.visual_gen and packed_vae_token_indexes is not None and has_visual_generation:
                num_vae_tokens = packed_vae_token_indexes.shape[0]
                if num_vae_tokens > 0 and self.config.cross_modal_recon_vae_mask_prob > 0:
                    vae_mask = torch.rand(num_vae_tokens, device=device) < self.config.cross_modal_recon_vae_mask_prob
                    if not vae_mask.any():
                        vae_mask[torch.randint(0, num_vae_tokens, (1,), device=device)] = True

        if vit_mask is None and self.config.visual_und and packed_vit_tokens is not None and packed_vit_token_indexes is not None:
            vit_mask = torch.zeros(packed_vit_token_indexes.shape[0], dtype=torch.bool, device=device)
        if vae_mask is None and self.config.visual_gen and has_visual_generation and packed_vae_token_indexes is not None:
            vae_mask = torch.zeros(packed_vae_token_indexes.shape[0], dtype=torch.bool, device=device)
        ...
        if self.config.visual_und:
            ...
            if vit_mask is not None and vit_mask.any():
                packed_vit_token_embed[vit_mask] = 0
            packed_sequence[packed_vit_token_indexes] = packed_vit_token_embed

        if has_visual_generation:
        ...
        if has_visual_generation:
            ...
            mse_target = target[has_mse]
            mse = (packed_mse_preds - mse_target) ** 2

        if self.config.cross_modal_recon and self.training:
            recon_losses = []
            if self.config.visual_und and vit_mask is not None and vit_mask.any():
                recon_vit = self.vit_recon_head(
                    last_hidden_state[packed_vit_token_indexes[vit_mask]]
                )
                recon_losses.append((recon_vit - packed_vit_tokens[vit_mask]) ** 2)
            if self.config.visual_gen and has_visual_generation and vae_mask is not None and vae_mask.any():
                recon_vae = self.vae_recon_head(
                    last_hidden_state[packed_vae_token_indexes[vae_mask]]
                )
                recon_losses.append((recon_vae - packed_latent_clean[vae_mask]) ** 2)
            if len(recon_losses) > 0:
                cross_modal_recon = torch.cat([loss.mean(-1) for loss in recon_losses], dim=0)
        ...
        output = dict(mse=mse, ce=ce, cross_modal_recon=cross_modal_recon)
        has_dynamic_mse_aux = (
            getattr(self.language_model.model, "_training_generation_aux", None)
            is not None
            or getattr(self.language_model.model, "_training_fusion_aux", None)
            is not None
        )
        if mse_target is not None and has_dynamic_mse_aux:
            output["mse_target"] = mse_target
        return output
    ...
    def generate_text(
        ...
        if mode != "und":
            raise ValueError(
                "Text generation uses the understanding branch (mode='und'); "
                "the generation branch is reserved for image latents."
            )
        step = 0
        generated_sequence = []
        ...
        while step < max_length:
            ...
            if (
                valid_vocab_size is not None
                and valid_vocab_size < pred_logits.shape[-1]
            ):
                pred_logits[..., valid_vocab_size:] = -torch.inf
