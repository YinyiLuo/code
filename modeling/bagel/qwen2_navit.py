...
try:
...
except ImportError:  # pragma: no cover - exercised on minimal CUDA hosts
    def flash_attn_varlen_func(
        *,
        ...
        """Small variable-length SDPA fallback for inference-only paths.

        Training uses the repository's packed SDPA/flex path, but this module
        imports FlashAttention unconditionally because the inference cache
        path uses it.  Keeping the fallback here makes the checkout runnable
        on CUDA images that do not ship a wheel matching their PyTorch build.
        """
        ...
        for index in range(cu_seqlens_q.numel() - 1):
            ...
            # Qwen2 uses grouped-query attention (28 query heads, 4 KV
            # heads).  PyTorch SDPA does not implicitly broadcast KV heads
            # on all supported versions, while FlashAttention does.  Match
            # FlashAttention's behavior in the fallback path.
            if query.shape[1] != key.shape[1]:
                if query.shape[1] % key.shape[1] != 0:
                    raise RuntimeError(
                        "Grouped-query attention requires the query-head "
                        "count to be divisible by the KV-head count"
                    )
...
def _set_training_route_token_indexes(module: nn.Module, indexes) -> None:
    """Expose packed positions to a Training-wrapped FFN when present.

    Legacy Qwen MLPs do not have this attribute and remain untouched.  The
    routed wrapper uses it to expand per-sample task metadata to the exact
    text/vision rows passed into that FFN.
    """

    if hasattr(module, "_training_route_token_indexes"):
        module._training_route_token_indexes = indexes
...
class Qwen2MoTDecoderLayer(nn.Module):
    def __init__(
        ...
        # These optional scalar residual gates are initialized at zero:
        # 1 + tanh(0) == 1.  They are conditional so legacy checkpoints do not
        # acquire missing parameters when Accelerate constructs on meta device.
        if getattr(config, "use_residual_gates", False):
            self.und_attn_gate = nn.Parameter(torch.zeros(1))
            self.gen_attn_gate = nn.Parameter(torch.zeros(1))
            self.und_mlp_gate = nn.Parameter(torch.zeros(1))
            self.gen_mlp_gate = nn.Parameter(torch.zeros(1))

    @staticmethod
    def _gate(raw_gate: torch.Tensor, like: torch.Tensor) -> torch.Tensor:
        return (1.0 + torch.tanh(raw_gate)).to(
            device=like.device,
            dtype=like.dtype,
        )

    def _residual_gate(self, name: str, like: torch.Tensor) -> torch.Tensor:
        raw_gate = getattr(self, name, None)
        if raw_gate is None:
            return like.new_ones(1)
        return self._gate(raw_gate, like)
    ...
    def forward_train(
        ...
        packed_sequence_[packed_und_token_indexes] = packed_sequence_[packed_und_token_indexes] * self._residual_gate(
            "und_attn_gate", packed_sequence_[packed_und_token_indexes]
        )
        packed_sequence_[packed_gen_token_indexes] = packed_sequence_[packed_gen_token_indexes] * self._residual_gate(
            "gen_attn_gate", packed_sequence_[packed_gen_token_indexes]
        )
        ...
        residual = packed_sequence
        packed_sequence_ = packed_sequence.new_zeros(packed_sequence.shape)
        _set_training_route_token_indexes(self.mlp, packed_und_token_indexes)
        ...
        _set_training_route_token_indexes(self.mlp_moe_gen, packed_gen_token_indexes)
        ...
        packed_sequence_[packed_und_token_indexes] = packed_sequence_[packed_und_token_indexes] * self._residual_gate(
            "und_mlp_gate", packed_sequence_[packed_und_token_indexes]
        )
        packed_sequence_[packed_gen_token_indexes] = packed_sequence_[packed_gen_token_indexes] * self._residual_gate(
            "gen_mlp_gate", packed_sequence_[packed_gen_token_indexes]
        )
        packed_sequence = residual + packed_sequence_
    ...
    def forward_inference(
        ...
        if not enable_taylorseer or (enable_taylorseer and self.current['type'] == 'full'):
            ...
            if mode == "und":
                packed_query_sequence = packed_query_sequence * self._residual_gate(
                    "und_attn_gate",
                    packed_query_sequence,
                )
            elif mode == "gen":
                packed_query_sequence[packed_text_indexes] = packed_query_sequence[packed_text_indexes] * self._residual_gate(
                    "und_attn_gate",
                    packed_query_sequence[packed_text_indexes],
                )
                packed_query_sequence[packed_vae_token_indexes] = packed_query_sequence[packed_vae_token_indexes] * self._residual_gate(
                    "gen_attn_gate",
            ...
            if mode == "und":
                packed_query_sequence = self.post_attention_layernorm(packed_query_sequence)
                _set_training_route_token_indexes(self.mlp, packed_query_indexes)
                packed_query_sequence = self.mlp(packed_query_sequence)
                packed_query_sequence = packed_query_sequence * self._residual_gate(
                    "und_mlp_gate",
                    packed_query_sequence,
                ...
                _set_training_route_token_indexes(self.mlp, packed_text_indexes)
                packed_query_sequence_[packed_text_indexes] = self.mlp(packed_text_query_sequence)
                _set_training_route_token_indexes(self.mlp_moe_gen, packed_vae_token_indexes)
                packed_query_sequence_[packed_vae_token_indexes] = self.mlp_moe_gen(packed_vae_query_sequence)
                packed_query_sequence_[packed_text_indexes] = packed_query_sequence_[packed_text_indexes] * self._residual_gate(
                    "und_mlp_gate",
                    packed_query_sequence_[packed_text_indexes],
                )
                packed_query_sequence_[packed_vae_token_indexes] = packed_query_sequence_[packed_vae_token_indexes] * self._residual_gate(
                    "gen_mlp_gate",
