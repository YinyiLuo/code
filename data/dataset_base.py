...
class PackedDataset(torch.utils.data.IterableDataset):
    ...
    def build_datasets(self, datasets_metainfo, data_status):
        ...
        for grouped_dataset_name, dataset_args in datasets_metainfo.items():
            ...
            dataset_cls = DATASET_REGISTRY[grouped_dataset_name]
            if dataset_cls.__name__ == "SftJSONLIterableDataset":
                dataset_args.setdefault('source_dataset_names', list(dataset_names))
            ...
            for item in dataset_names:
                ...
                if 'image_dir' in meta_info.keys():
                    if 'image_dir_list' not in dataset_args.keys():
                        dataset_args['image_dir_list'] = [meta_info['image_dir']]
                    else:
                        dataset_args['image_dir_list'].append(meta_info['image_dir'])
    ...
    def to_tensor(self, sequence_status):
        ...
        if not self.use_flex:
            ...
            if pad_len < 0:
                raise ValueError(
                    f"Packed sequence length {sequence_len} exceeds "
                    f"max_num_tokens {self.max_num_tokens}. Reduce mandatory "
                    "groups or increase max_num_tokens."
                )
